        ...
        if courses_cursor:
            ...
            if subject:
                ...
                if course and "tasks" in course and course["tasks"]:
                    ...
                    # Add View Mode Radio with Single Student Analysis
                    view_mode = col2.radio(
                        "Select View Mode",
                        ["📋 General", "📈 Visualizations", "👤 Single Student Analysis"],
                        horizontal=True,
                        key="stats_view_mode"
                    )
                    ...
                    if view_mode == "👤 Single Student Analysis":
                        display_single_student_analysis(col2, course, academic_year, department, subject)
                    else:
                        if task_dates:
                            selected_date = col2.selectbox(
                                "Select Task Date",
...
def display_single_student_analysis(col2, course, academic_year, department, subject):
    """Display detailed analysis for a single student"""
    ...
    # Student selection
    student_names = [s.get("student_name", f"Student {i+1}") for i, s in enumerate(students)]
    selected_student_name = col2.selectbox(
        "Select Student",
        student_names,
    ...
    selected_student = next(
        (s for s in students if s.get("student_name") == selected_student_name),
        None
    )
    
    if not selected_student:
        col2.warning("Student not found")
        return
    ...
    if not all_tasks:
        col2.info("No tasks available for this course")
        return
    ...
    for task in all_tasks:
        for stat in task.get("stats", []):
            if stat.get("student_name") == selected_student_name:
                student_tasks.append({
                    "task_date": task.get("task_date", ""),
                    "task_name": task.get("task_name", ""),
                    "total_marks": task.get("total_marks", 0),
                    "status": stat.get("status", "incomplete"),
                    "obtained_marks": stat.get("total_marks_obtained", 0),
                    "completed_date": stat.get("completed_date", None),
                    "started_at": stat.get("started_at", None),
                    "ended_at": stat.get("ended_at", None)
                })
                break
    
    if not student_tasks:
        col2.info(f"No task data found for {selected_student_name}")
        return
    
    # Create DataFrame
    df_student = pd.DataFrame(student_tasks)
    
    # Analysis Mode Selection
    analysis_mode = col2.pills(
        "Select Analysis Mode",
        ["Complete Stats", "Specific Dates"],
        key="single_student_mode"
    )
    
    if analysis_mode == "Specific Dates":
        # Date range selection
        col2.subheader("📅 Date Range Selection", divider="blue")
        
        from_date = col2.date_input("From Date", key="single_student_from_date")
        to_date = col2.date_input("To Date", key="single_student_to_date")
        
        # Toggle to display stats
        show_stats = col2.toggle("📊 Show Statistics", key="single_student_toggle")
        
        if show_stats:
            # Filter tasks by date range
            filtered_tasks = df_student[
                (pd.to_datetime(df_student["task_date"]) >= pd.to_datetime(from_date)) &
                (pd.to_datetime(df_student["task_date"]) <= pd.to_datetime(to_date))
            ]
            
            if filtered_tasks.empty:
                col2.warning(f"No tasks found between {from_date} and {to_date}")
                return
            
            display_student_stats(col2, filtered_tasks, selected_student, course, from_date, to_date)
        else:
            col2.info("👆 Toggle the switch above to view statistics")
    
    else:  # Complete Stats
        display_student_stats(col2, df_student, selected_student, course, None, None)

def display_student_stats(col2, df_student, student, course, from_date=None, to_date=None):
    """Display comprehensive student statistics"""
    
    # Determine date range text
    date_range_text = "Complete Statistics"
    if from_date and to_date:
        date_range_text = f"Statistics from {from_date} to {to_date}"
    
    col2.subheader(f"📊 {date_range_text}", divider="blue")
    
    # Basic student info
    col2.write(f"**Student:** {student.get('student_name', 'Unknown')}")
    col2.write(f"**Roll Number:** {student.get('student_roll_number', 'Unknown')}")
    col2.write(f"**Gender:** {student.get('student_gender', 'Unknown')}")
    
    col2.divider()
    
    # Summary metrics
    total_tasks = len(df_student)
    completed_tasks = len(df_student[df_student["status"] == "completed"])
    incomplete_tasks = total_tasks - completed_tasks
    
    col_metrics1, col_metrics2, col_metrics3, col_metrics4 = col2.columns(4)
    col_metrics1.metric("📝 Total Tasks", total_tasks)
    col_metrics2.metric("✅ Completed", completed_tasks)
    col_metrics3.metric("⏳ Incomplete", incomplete_tasks)
    
    # Calculate average marks for completed tasks
    completed_df = df_student[df_student["status"] == "completed"]
    if not completed_df.empty:
        avg_marks = completed_df["obtained_marks"].mean()
        avg_percentage = (avg_marks / completed_df["total_marks"].mean() * 100) if completed_df["total_marks"].mean() > 0 else 0
        col_metrics4.metric("📈 Avg Score", f"{avg_marks:.1f}/{completed_df['total_marks'].mean():.0f}", 
                           delta=f"{avg_percentage:.1f}%")
    else:
        col_metrics4.metric("📈 Avg Score", "No completions")
    
    col2.divider()
    
    # Completed Tasks Table
    col2.subheader("✅ Completed Tasks", divider="green")
    completed_tasks_df = df_student[df_student["status"] == "completed"]
    
    if not completed_tasks_df.empty:
        completed_display = completed_tasks_df[["task_date", "task_name", "obtained_marks", "total_marks"]].copy()
        # Add Pass/Fail column (35% passing criteria)
        completed_display["percentage"] = (completed_display["obtained_marks"] / completed_display["total_marks"] * 100).round(1)
        completed_display["Pass/Fail"] = completed_display["percentage"].apply(lambda x: "✅ Pass" if x >= 35 else "❌ Fail")
        completed_display = completed_display[["task_date", "task_name", "obtained_marks", "total_marks", "Pass/Fail"]]
        col2.dataframe(completed_display, use_container_width=True)
        
        # Calculate totals
        total_obtained = completed_display["obtained_marks"].sum()
        total_possible = completed_display["total_marks"].sum()
        col2.write(f"**Total Marks Obtained:** {total_obtained} / {total_possible}")
    else:
        col2.info("No completed tasks found")
    
    col2.divider()
    
    # Incomplete Tasks Table
    col2.subheader("⏳ Incomplete Tasks", divider="orange")
    incomplete_tasks_df = df_student[df_student["status"] == "incomplete"]
    
    if not incomplete_tasks_df.empty:
        incomplete_display = incomplete_tasks_df[["task_date", "task_name", "total_marks"]].copy()
        col2.dataframe(incomplete_display, use_container_width=True)
        
        total_incomplete_marks = incomplete_display["total_marks"].sum()
        col2.write(f"**Total Possible Marks in Incomplete Tasks:** {total_incomplete_marks}")
    else:
        col2.info("No incomplete tasks found")
    
    col2.divider()
    
    # Track Information
    col2.subheader("📈 Study Track Information", divider="blue")
    
    # Get student's track data
    track_data = student.get("track", [])
    
    if track_data:
        # Track stats
        col2.write(f"**Total Study Sessions:** {len(track_data)}")
        
        # Date range from track
        track_dates = [t.get("date", "") for t in track_data if t.get("date")]
        if track_dates:
            col2.write(f"**First Session:** {min(track_dates)}")
            col2.write(f"**Last Session:** {max(track_dates)}")
        ...
        for entry in track_data:
            study_times = entry.get("study_time", [])
            for time_range in study_times:
                if " - " in time_range:
        ...
        hours = total_study_seconds // 3600
        minutes = (total_study_seconds % 3600) // 60
        seconds = total_study_seconds % 60
        
        col2.write(f"**Total Study Time:** {hours}h {minutes}m {seconds}s")
        col2.write(f"**Total Time Entries:** {len(all_time_ranges)}")
        
        # Show track data in expandable section
        with col2.expander("📋 View Detailed Track Data"):
            track_df = pd.DataFrame(track_data)
            col2.dataframe(track_df, use_container_width=True)
    else:
        col2.info("No study track data available for this student")
    
    col2.divider()
    
    # Additional metrics
    col2.subheader("📊 Performance Summary", divider="green")
    
    if not completed_tasks_df.empty:
        # Calculate passing rate
        pass_count = len(completed_tasks_df[completed_tasks_df["obtained_marks"] / completed_tasks_df["total_marks"] * 100 >= 35])
        pass_rate = (pass_count / len(completed_tasks_df) * 100) if len(completed_tasks_df) > 0 else 0
        
        col_extra1, col_extra2, col_extra3 = col2.columns(3)
        col_extra1.metric("🎯 Pass Rate", f"{pass_rate:.1f}%")
        
        # Best and worst performance
        best_task = completed_tasks_df.loc[completed_tasks_df["obtained_marks"].idxmax()] if not completed_tasks_df.empty else None
        worst_task = completed_tasks_df.loc[completed_tasks_df["obtained_marks"].idxmin()] if not completed_tasks_df.empty else None
        
        if best_task is not None:
            col_extra2.metric("🏆 Best Score", 
                             f"{best_task['obtained_marks']}/{best_task['total_marks']}",
                             delta=f"{(best_task['obtained_marks']/best_task['total_marks']*100):.1f}%")
        
        if worst_task is not None:
            col_extra3.metric("📉 Lowest Score",
                             f"{worst_task['obtained_marks']}/{worst_task['total_marks']}",
                             delta=f"{(worst_task['obtained_marks']/worst_task['total_marks']*100):.1f}%")
        
        # Performance trend (mini visualization)
        if len(completed_tasks_df) > 1:
            col2.write("**Performance Trend:**")
            # Create simple bar chart for task performance
            import plotly.express as px
            trend_df = completed_tasks_df.sort_values("task_date")
            trend_df["percentage"] = (trend_df["obtained_marks"] / trend_df["total_marks"] * 100).round(1)
            
            fig = px.bar(
                trend_df,
                x="task_date",
                y="percentage",
                title="Task Performance Trend",
                labels={"percentage": "Score (%)", "task_date": "Task Date"},
                color="percentage",
        ...
        col2.info("Complete at least one task to see performance summary")
...
def display_visualizations(col2, date_tasks, task_names, course, academic_year, department, subject):
    ...
    if selected_viz == "📊 Task Completion Rates":
        ...
        col2.plotly_chart(fig, use_container_width=True)
        col2.info("💡 Shows pass/fail distribution for each task (35% passing threshold)")
        ...
        total_completions = len(df)
        avg_score = df["percentage"].mean() if not df.empty else 0
        pass_rate = len(df[df["percentage"] >= 35]) / len(df) * 100 if len(df) > 0 else 0
