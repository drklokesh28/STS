                            ...
                            if submit_button:
                                if not password:
                                    ...
                                    if course_data:
                                        ...
                                        st.session_state["start_time"] = datetime.now(ZoneInfo("Asia/Kolkata")).strftime("%I:%M:%S %p")
...
def show_sidebar():
    with st.sidebar:
        if st.session_state.get("logged_in", False):
            ...
            st.caption(f"📚 {st.session_state.get('subject', '')}")
            st.caption(f"🕐 Login Time: {st.session_state.get('start_time', '')}")
            st.divider()
